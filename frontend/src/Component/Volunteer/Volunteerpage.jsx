...
function StatCard({ icon, value, label }) {
  ...
  return (
    <div className="flex flex-col items-center justify-center p-4 bg-[#F8F9FA] rounded-2xl border border-zinc-100 w-full shadow-sm">
      ...
      <span className="text-2xl font-black text-[#1A1A1A]">{counted.toLocaleString()}</span>
      <span className="text-[9px] text-zinc-400 uppercase tracking-widest font-bold">{label}</span>
    </div>
  );
}
...
function CenterCard({ center, onVolunteer }) {
  ...
  return (
    <motion.div layout className="bg-white rounded-[24px] border border-zinc-200 shadow-sm overflow-hidden group hover:shadow-xl transition-all duration-500 text-black">
      <div className="h-[6px] bg-gradient-to-r from-[#2D4A35] to-[#9DC3A0]" />
      <div className="p-6">
        <div className="flex items-start justify-between mb-4">
          <div className="flex items-center gap-3">
            <div className="w-12 h-12 rounded-2xl bg-[#F4F0E8] flex items-center justify-center text-2xl group-hover:scale-110 transition-transform">{center.emoji}</div>
            <div>
        ...
        <div className="mb-5">
          ...
          <div className="h-2 bg-zinc-100 rounded-full overflow-hidden">
            <motion.div initial={{ width: 0 }} animate={{ width: `${fillPct}%` }} className="h-full bg-[#2D4A35]" />
          </div>
        </div>

        <button onClick={() => setExpanded(!expanded)} className="w-full text-[11px] font-bold text-[#2D4A35] py-2 rounded-lg hover:bg-zinc-50 transition-colors mb-4 uppercase">
          {expanded ? "Collapse Details ▲" : "View Full Details ▼"}
        </button>

        <AnimatePresence>
          {expanded && (
            <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden border-t border-zinc-100 pt-4 space-y-4 text-[12px] text-zinc-600">
        ...
        <button onClick={() => onVolunteer(center)} disabled={!isActive} className={`w-full py-4 mt-2 rounded-xl font-bold text-sm transition-all ${isActive ? "bg-[#2D4A35] text-white hover:bg-black shadow-lg" : "bg-zinc-100 text-zinc-300"}`}>
          BECOME A VOLUNTEER
        </button>
      </div>
    </motion.div>
  ...
  return (
    <div className="min-h-screen bg-white text-[#1A1A1A] font-sans overflow-x-hidden">
      ...
      <section className="relative min-h-screen flex items-center px-6 lg:px-20 py-20">
        <div className="max-w-[1400px] mx-auto grid lg:grid-cols-2 gap-12 items-center z-10">
          ...
          <motion.div initial={{ opacity: 0, x: -30 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8 }}>
            ...
            <h1 className="text-[clamp(48px,8vw,90px)] font-black leading-[0.9] tracking-tight mb-6 text-[#1A1A1A]">
              Swap <span className="text-[#2D4A35]">Better</span>,<br />Live Together.
            </h1>
            <p className="text-lg text-zinc-500 max-w-[500px] leading-relaxed mb-10">
              Join Sri Lanka's fastest-growing community swap network. Volunteer at a center near you — sort, connect, and help items find new homes.
            </p>
            
            <div className="flex flex-wrap gap-4 mb-16">
              <button onClick={() => document.getElementById('browse').scrollIntoView({ behavior: 'smooth' })} className="bg-[#F2C94C] text-black px-8 py-4 rounded-xl font-black text-sm hover:scale-105 active:scale-95 transition-all shadow-md">
                BROWSE CENTERS
              </button>
              <button onClick={() => navigate("/volunteer")} className="bg-[#2D4A35] text-white px-8 py-4 rounded-xl font-black text-sm hover:bg-black transition-all shadow-md">
                BECOME VOLUNTEER
              </button>
            </div>
      ...
      <section id="browse" className="py-24 px-6 max-w-[1200px] mx-auto">
        <div className="mb-12">
          <h2 className="text-4xl font-black tracking-tight mb-4">Find Your Local Hub</h2>
          <p className="text-zinc-400 font-medium">Currently showing our active SwapNest locations across Sri Lanka.</p>
        </div>
        ...
        <motion.div layout className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {CENTERS.map(center => (
            <CenterCard 
              key={center.id} 
              center={center} 
              onVolunteer={(c) => navigate("/volunteer", { state: { center: c } })} 
            />
          ))}
        </motion.div>
      </section>
