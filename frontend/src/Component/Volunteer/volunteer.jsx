...
/** Same base as `services/api.js` - path is `/volunteers`, not `/api/volunteers` again. */
const apiBaseUrl = (import.meta.env.VITE_API_URL || "http://localhost:5000/api").replace(/\/$/, "");

const Volunteer = ({ isOpen, onClose, center }) => {
  ...
  const selectedCenter = center || location.state?.center;
    ...
    async function loadCenters() {
      ...
      try {
        ...
        if (!cancelled) {
          ...
          try {
            const response = await fetch(`${apiBaseUrl}/centers`);
  ...
  const validateStep = () => {
    ...
    if (currentStep === 1) {
      ...
      if (!formData.password.trim()) missing.push("Password");
        ...
        setSubmitError("Password must contain at least one uppercase letter, one lowercase letter, one number, and one special character");
        return ["Password validation"];
      }
    }
    if (currentStep === 2) {
    ...
    if (currentStep === totalSteps) {
      ...
      (async () => {
        try {
          ...
          const res = await fetch(`${apiBaseUrl}/volunteers`, {
            method: "POST",
  ...
  const handleClose = () => {
    onClose();
    // Reset state so it opens on first step next time
    setTimeout(() => {
      setCurrentStep(1);
    }, 300);
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
      <div className="w-full max-w-4xl max-h-[90vh] overflow-hidden bg-white rounded-3xl shadow-2xl border border-[#D0C9BA] flex flex-col relative">
        <button 
          onClick={handleClose}
          className="absolute top-4 right-4 w-10 h-10 flex items-center justify-center rounded-full bg-white/50 hover:bg-gray-100 text-gray-500 hover:text-black transition-colors z-10"
        >
          ✕
        </button>
        ...
        <div className="p-8 md:p-12 overflow-y-auto">
          ...
          {currentStep === 1 && (
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <InputField label="Password" required type="password" placeholder="Create a strong password" id="password" value={formData.password} onChange={handleInputChange} />
                ...
                <div className="md:col-span-2 rounded-xl border border-[#D0C9BA] bg-zinc-50 px-4 py-3 text-sm text-zinc-600">
                  <span className="font-semibold text-zinc-800">Account type:</span> Volunteer (fixed)
                </div>
          ...
          {currentStep === 3 && (
            <div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-8">
                <CheckCard icon="??" label="Communication" name="skills" value="communication" checked={formData.skills.includes("communication")} onChange={handleInputChange} />
          ...
          {currentStep === 4 && (
            <div>
              <div className="text-xs font-bold text-[#C4622D] uppercase tracking-widest mb-2">?? Step 4 of 5</div>
...
const SelectField = ({ label, required, id, value, onChange, options, disabled }) => (
  <div className="flex flex-col gap-2">
    <label className="text-sm font-semibold">
      {label} {required ? <span className="text-red-500">*</span> : <span className="text-gray-400 font-normal text-xs">(optional)</span>}
    </label>
    <select
      ...
      onChange={onChange}
      disabled={disabled}
      className="px-4 py-3 rounded-xl border border-[#D0C9BA] bg-white focus:ring-2 focus:ring-[#7A9E7E] outline-none transition-all cursor-pointer disabled:opacity-60 disabled:cursor-not-allowed"
    >
