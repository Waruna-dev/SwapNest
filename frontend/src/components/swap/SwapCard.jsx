...
const SwapCard = ({ swap, onStatusChange }) => {
  ...
  const isRequester =
    currentUser?._id === swap.requesterId?._id ||
    currentUser?._id === swap.requesterId;
  const isOwner =
    currentUser?._id === swap.requestedItem?.ownerId?._id ||
  ...
  const handleAccept = async () => {
    if (
      !confirm(
        "Accept this swap request? The item will no longer be available.",
      )
    )
      return;
  ...
  const handleReject = async () => {
    if (!confirm("Reject this swap request? The requester will be notified."))
      return;
    setLoading(true);
    try {
  ...
  const handleCancel = async () => {
    if (!confirm("Cancel this swap request? This action cannot be undone."))
      return;
  ...
  const getSwapPreview = () => {
    if (swap.swapType === "swap-with-cash") {
      const direction =
        swap.cashDetails?.whoPays === "requester-pays-owner"
        ...
        <div className="mt-3 space-y-2">
          <div className="flex items-center gap-2 text-sm text-gray-600">
            ...
            {isRequester ? (
              ...
              <span>
                From: <span className="font-medium">{swap.requesterName}</span>
              </span>
            )}
          </div>
