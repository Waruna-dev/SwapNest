...
import { Trash2, Edit, ShieldAlert, ShieldCheck, User as UserIcon, X, AlertTriangle } from 'lucide-react';
...
const ManageUsers = () => {
  ...
  const handleUpdateSubmit = async (e) => {
    ...
    try {
      const { data } = await API.put(`/admin/users/${editingUser._id}`, editForm, getConfig());
    ...
    <div className="p-6 lg:p-8 relative">
      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">View, edit, and remove SwapNest members.</p>
        </div>
          ...
          <div className="bg-white rounded-2xl w-full max-w-sm shadow-2xl p-6 text-center transform transition-all">
            ...
            <div className="flex gap-3">
              <button 
                ...
                className="flex-1 py-2.5 bg-gray-100 hover:bg-gray-200 text-gray-800 font-bold rounded-lg transition"
              >
                Cancel
              </button>
              <button 
                onClick={executeDelete} 
                className="flex-1 py-2.5 bg-red-600 hover:bg-red-700 text-white font-bold rounded-lg transition"
              >
                Yes, Delete
              </button>
            </div>
