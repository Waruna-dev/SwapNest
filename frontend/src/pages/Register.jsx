...
const Register = () => {
  ...
  const handleRegister = async (e) => {
    ...
    // Backend Safeguard: Check passwords
  ...
  return (
    <div className="bg-background font-body text-on-surface antialiased min-h-screen flex flex-col selection:bg-secondary-fixed selection:text-on-secondary-fixed overflow-hidden">
      ...
      {/* CHANGED: Removed min-h-screen from here so it fits exactly */}
      <main className="flex-grow flex flex-col md:flex-row h-screen pt-[72px]">
        
        {/* Left Side: Editorial Content */}
        {/* CHANGED: h-full to lock it perfectly into the left side */}
        ...
        {/* CHANGED: Removed justify-center, added overflow-y-auto so the form scrolls nicely if it gets too long, and adjusted padding */}
        <section className="w-full md:w-7/12 lg:w-1/2 flex items-start justify-center p-6 md:p-12 lg:p-16 bg-surface-container-low h-full overflow-y-auto">
          <div className="w-full max-w-lg mt-4 md:mt-10 pb-20">
            ...
            <form onSubmit={handleRegister} className="space-y-6">
              ...
              <div className="space-y-5">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  ...
                  <div className="relative group">
                    <label className="block text-[11px] uppercase tracking-widest font-bold text-on-surface-variant mb-2 ml-1">Confirm Password</label>
                    <input 
                      className="w-full bg-surface-container-high border-none rounded-2xl px-6 py-4 text-on-surface focus:ring-2 focus:ring-primary/20 transition-all placeholder:text-on-surface-variant/40 font-medium outline-none" 
                      placeholder="••••••••••••" 
                      type="password"
              ...
              <div className="pt-6">
                <button 
                  type="submit"
                  disabled={isLoading}
                  className="w-full py-4 rounded-full bg-secondary text-on-secondary font-headline font-bold text-lg hover:bg-[#822800] transition-all shadow-lg shadow-secondary/20 hover:scale-[1.01] active:scale-[0.98] disabled:opacity-70 disabled:cursor-not-allowed"
                >
