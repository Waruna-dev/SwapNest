...
const createSwapRequest = async (req, res) => {
  try {
    ...
    if (post.ownerId.toString() === requesterId)
      return res
        .status(404)
        .json({ success: false, message: "Cannot swap your own item" });
...
const cancelSwapRequest=async(req,res)=>{
    try{
        const swap=await Swap.findById(req,params.id);
        if(!swap) return red.status(404).json({success:false,message:'swap not found'});
...
const getAllSwaps=async(req,res)=>{
    try{
        ...
        if(sort==='status')sortOption={status:1,createedAt:-1};

        const swaps= (await Swap.find(filter)).toSorted(sortOption);
